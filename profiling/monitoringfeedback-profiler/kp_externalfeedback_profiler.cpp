...
namespace ExternalFeedbackProfiler {
...
// TODO: do some listening activity to get a simple message from external
// feedback library, and do an action based on it
...
void kokkosp_init_library(const int loadSeq, const uint64_t interfaceVer,
  ...
  if (verbose_env &&
    ...
    KokkosTools::ExternalFeedbackProfiler::ExternalFeedbackProfilerState::get_instance()
        .set_verbose_enabled(true);
  }
  std::cout << "Kokkos External Feedback Profiler: Initializing with load sequence "
            << loadSeq << " and interface version " << interfaceVer
            << std::endl;
  std::cout << "Kokkos External Feedback Profiler: Library initialized" << std::endl;
}

// Library finalize
void kokkosp_finalize_library() {
  std::cout << "Kokkos External Feedback Profiler: Finalizing library" << std::endl;
  std::string prefix = KokkosTools::ExternalFeedbackProfiler::generate_prefix();
  auto all_timings   = KokkosTools::ExternalFeedbackProfiler::get_all_timings();
  KokkosTools::ExternalFeedbackProfiler::print_all_timings_summary(
      std::cout, all_timings.begin(), all_timings.end());
  KokkosTools::ExternalFeedbackProfiler::export_all_timings_csv(
      all_timings, prefix + "_timing_data.csv");
  std::cout << "Kokkos External Feedback Profiler: Library finalized" << std::endl;
}
...
void kokkosp_begin_parallel_for(const char* name, const uint32_t devID,
  ...
  (void)devID;
  uint64_t new_id = KokkosTools::ExternalFeedbackProfiler::generate_new_region_id();
  *kID            = new_id;
...
void kokkosp_end_deep_copy() {
  KokkosTools::ExternalFeedbackProfiler::end_region_by_type(
      KokkosTools::ExternalFeedbackProfiler::RegionType::DeepCopy);
  KokkosTools::ExternalFeedbackProfiler::log_verbose(
      "Kokkos External Feedback Profiler: Ended deep copy");
}
